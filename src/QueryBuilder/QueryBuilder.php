...
class QueryBuilder
{
    ...
    public function bulkUpdate(array $attributes)
    {
        try {
            ...
            foreach ($attributes as $key => $attribute) {
                ...
                $i++;
                $j = 0;
                if (property_exists($instance->getCalledClass(), 'updated_at')) {
    ...
    {
        return property_exists($this->getCalledClass(), 'deleted_at') && $this->withTrashed == false;
    }
    ...
    private function getTrashed()
    {
        ...
        return $this->table . '.deleted_at IS NULL';
    }

    private function getSubQueryTrashed($where, $current)
    {
        if (!$this->checkSubQueryTrashed($where)) {
            return null;
        }
        return $current['table'] . '.deleted_at IS NULL';
    }
    ...
    {
        return $this->getBaseSQL($this->getSelect());
    }

    private function deleteSQL()
    {
        $checkTrash = property_exists($this->getCalledClass(), 'deleted_at');
        $updateSQL = "UPDATE " . $this->table . " SET deleted_at='" . now() . "'";
        $deleteSQL = $checkTrash ? $updateSQL : $this->makeDelete();
        return $this->getBaseSQL($deleteSQL);
    }

    private function forceDeleteSQL()
    {
        return $this->getBaseSQL($this->makeDelete());
    }

    private function restoreSQL()
    {
        return $this->getBaseSQL($this->makeRestore());
    }
